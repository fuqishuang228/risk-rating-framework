...
Label_discription=pd.read_csv("./data/all_hash_0_hop_643_labels.csv")
discription=Label_discription['discription']
...
nodes = Account_Info["index"]
node_label = Account_Info["label"]
edges = Transaction_Info["Tx_id"]
Tx_Score = Transaction_Info["de_anonymous_score"]
reliable_0=Account_Info["supervised_reliable_0"]
...
for node_t in nodes:
    trust[node_t] = 0.5
    reliable[node_t] = reliable_0[node_t]
...
while iter < 400:
    ...
    for edge in edges:
        trust_value_all[Transaction_Info["index_to"][edge]] += Tx_Score[edge] * confidence[edge]
    ...
    for node in nodes:
        if  node_label[node] == 1:
            continue
        if node < 643:
            if discription[node]=='mining':
                continue
        ...
        if payer_txn > 0.0:
            ...
            reliable_for_node = reliable_0[node]
...
fw = open("./results/20220205-Account-trust-reliabl-risk-supervised-riskprop.csv", "w")
...
fw_confidence = open("./results/20220205-Transaction-score-confidence-supervised-riskprop.csv", "w")
fw_confidence.write("Transaction,score,confidence\n")

for edge in edges:
    fw_confidence.write("%s,%s,%s\n" % (str(edge), str(confidence[edge]), str(Tx_Score[edge])))
